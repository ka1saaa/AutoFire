...
class AutoFireApp:
    ...
    def _build_contacts_tab(self) -> None:
        ...
        ttk.Button(actionbar, text="新增", command=self.add_contact_dialog).pack(side=tk.LEFT, padx=3)
        ttk.Button(actionbar, text="从剪贴板导入", command=self.import_clipboard).pack(side=tk.LEFT, padx=3)
        ttk.Button(actionbar, text="从截图识别", command=self.import_image_contacts).pack(side=tk.LEFT, padx=3)
        ttk.Button(actionbar, text="全选", command=lambda: self.set_visible_selected(True)).pack(side=tk.LEFT, padx=3)
        ttk.Button(actionbar, text="取消全选", command=lambda: self.set_visible_selected(False)).pack(side=tk.LEFT, padx=3)
        ttk.Button(actionbar, text="下一步：生成预览", command=self.generate_plan).pack(side=tk.LEFT, padx=12)
